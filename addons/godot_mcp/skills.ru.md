...
# Godot MCP MyPro — Навыки для ИИ-ассистентов

> Настройка Codex по умолчанию: скопируйте этот файл в `.codex/skills/godot-mcp-mypro/SKILL.md`, чтобы дать Codex полный контекст по эффективному использованию Godot MCP MyPro.

## Что такое Godot MCP MyPro?

Вам доступны 177 MCP-инструментов, которые напрямую подключаются к редактору Godot 4. Вы можете создавать сцены, писать скрипты, симулировать ввод игрока, инспектировать запущенные игры и многое другое — всё это без выхода пользователя из данного разговора. Все изменения проходят через систему UndoRedo Godot, поэтому пользователь всегда может нажать Ctrl+Z.
