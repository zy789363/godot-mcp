...
const SERVER_PORT = Number(process.env.MCP_TEST_PORT ?? '6506');

const projectPath = process.env.P01_PROJECT ?? process.argv[2] ?? DEFAULT_PROJECT;
const godotPath = process.env.GODOT_PATH ?? DEFAULT_GODOT;
const runId = process.env.MCP_TEST_RUN_ID ?? `run_${Date.now()}`;
const TEST_BASE = `res://mcp_mypro_test/${runId}`;
const reportDir = path.join(projectPath, 'docs', 'mcp-mypro', 'reports');
const reportPath = path.join(reportDir, 'p01_full_tool_report.json');
...
function isInfrastructureError(text) {
  return (
...
async function callTool(client, name, args = {}, options = {}) {
  ...
  try {
    ...
    const infraError = isError && isInfrastructureError(text);
    const ok = isError
      ? options.expectError === true && (!infraError || options.allowInfrastructureError === true)
      : true;
...
`;
  ...
  await callTool(client, 'get_resource_preview', { path: 'res://icon.svg', max_size: 64 }, { phase: 'resource', expectError: true });
  ...
  await callTool(client, 'execute_editor_script', { code: '_mcp_print("editor script ok")' }, { phase: 'editor' });
  ...
  await callTool(client, 'execute_game_script', { code: '_mcp_print("runtime script ok")' }, { phase: 'runtime' });
  ...
  await callTool(client, 'bake_navigation_mesh', { node_path: 'NavigationRegion3D' }, { phase: 'navigation', expectError: true });
  ...
  await callTool(client, 'set_tree_parameter', { node_path: 'AnimTree', parameter: 'conditions/mcp_test', value: true }, { phase: 'animation_tree', expectError: true });
...
async function main() {
  const client = new Client({ name: 'godot-mcp-mypro-p01-test', version: '1.0.0' });
  ...
  const report = {
    ...
    failedItems: failed,
    destructiveToolsPolicy: 'Deletion-capable tools were called only with missing/invalid parameters unless explicitly covered by non-deleting operations.',
    results,
  };
